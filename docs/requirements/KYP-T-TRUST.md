...
| REQ-KYP-T-TRUST-05 | Network policy MUST be default-deny between areas and between environments; allowed flows mirror declared model relations. The only permitted cross-environment flows are those derived from the model as env-scoped area → mediating service; an env-scoped area MUST never reach a store marked shared directly. | [ADR-0006](../../adr/0006-shared-zoned-data-layer.md) |
