...
| REQ-KYP-T-TRUST-02 | Secrets MUST be fetched at runtime under a workload identity with short-lived leases; every fetch is an audit event. Credentials into customer systems exist only in this tenant's plane. | ADR-0001 |
| REQ-KYP-T-TRUST-03 | Admission MUST verify signature and scan status before any artifact or model runs. | ADR-0004 |
