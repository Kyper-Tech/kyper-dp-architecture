...
|---|---|---|
| REQ-KYP-T-SRV-01 | The model runtime MUST load only promoted, signed model-registry entries; admission verifies before load. | ADR-0004 |
| REQ-KYP-T-SRV-02 | Every prediction MUST be written to the predictions store labelled with its environment. | ADR-0006 |
...
| REQ-KYP-T-SRV-05 | The drift monitor MUST read prod-labelled predictions via the query engine only. | ADR-0006 |
