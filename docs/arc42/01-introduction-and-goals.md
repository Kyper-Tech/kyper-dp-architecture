...
| 2 | Pull-only delivery; strict credential scoping (see note below) | A control-plane compromise must grant access into zero customer environments | plane structure and trust direction ([ADR-0001](../../adr/0001-plane-structure.md)) |
...
**Scope note on goal 2.** Of the six seams in the brief's trust rules, the
two that carry this goal follow different rules:
...
- *Ingestion path (tenant plane → customer systems):* direction is a
  ...
  trail (KYP-T-TRUST-05). The blast radius of any such credential is the
  one tenant it belongs to.

## Stakeholders

| Stakeholder | Concern | Where it is addressed |
|---|---|---|
| Customer (data owner) | Sovereignty, isolation, export on offboarding | tenant plane; lifecycle gap tracked in [open decisions in the design brief](../design-brief.md#open-decisions) |
...
| Kyper operations | Operate a fleet of tenant silos from one control plane | control plane ([ADR-0001](../../adr/0001-plane-structure.md)) |
